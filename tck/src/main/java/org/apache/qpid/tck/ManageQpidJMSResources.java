...
public class ManageQpidJMSResources
{
    ...
    {
        management("POST",
                   String.format(_queueApiClearQueueUrl, _virtualhostnode, _virtualhost, name),
                   Map.of());
    }

    private void managementCreateExchange(final String name, final Map<String, Object> arguments) throws IOException
    {
        management("PUT", String.format(_topicApiUrl, _virtualhostnode, _virtualhost, name), arguments);
    }

    private void managementDeleteQueue(final String name)
    {
        final HttpRequest request =
                newManagementRequest(String.format(_queueApiUrl, _virtualhostnode, _virtualhost, name))
                        .DELETE()
                        .build();
    ...
    {
        final HttpRequest request =
                newManagementRequest(String.format(_topicApiUrl, _virtualhostnode, _virtualhost, name))
                        .DELETE()
                        .build();
        executeManagement(request);
    }
    ...
    private void management(final String method, final String path, final Object obj) throws IOException
    {
        final HttpRequest request = newManagementRequest(path)
                ...
                .method(method,
                        HttpRequest.BodyPublishers.ofString(_objectMapper.writeValueAsString(obj),
