    ...
    private Map<String, Object> managementQueryBroker()
    {
        ...
        if (obj == null)
        {
            throw new IllegalStateException(
                    String.format("Unexpected null response from management query '%s'", request));
        }
        else if (obj instanceof Collection)
        {
            ...
            {
                throw new IllegalStateException(
                        String.format("Unexpected empty list response from management query '%s'", request));
            }
            obj = itr.next();
        }
        ...
        {
            throw new IllegalStateException(
                    String.format("Unexpected response '%s' from management query '%s'", obj, request));
        }
    }

    private void managementCreateQueue(final String name)
    {
        final HttpRequest request = newManagementRequest(String.format(_queueApiUrl,
                                                                       _virtualhostnode,
                                                                       _virtualhost,
                                                                       name))
    ...
    private Object executeManagement(final HttpRequest httpRequest)
    {
        try
        {
            final HttpResponse<byte[]> response =
                    _httpClient.send(httpRequest, HttpResponse.BodyHandlers.ofByteArray());
