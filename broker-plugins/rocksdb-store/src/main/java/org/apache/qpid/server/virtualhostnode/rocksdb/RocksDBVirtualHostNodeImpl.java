...
import org.apache.qpid.server.store.rocksdb.RocksDBConfigurationStore;
...
/**
 * Provides the RocksDB virtual host node implementation.
 * <br>
 * Thread-safety: safe for concurrent access by the configuration model.
 */
@ManagedObject(type = RocksDBVirtualHostNodeImpl.VIRTUAL_HOST_NODE_TYPE,
               category = false,
...
public class RocksDBVirtualHostNodeImpl extends AbstractStandardVirtualHostNode<RocksDBVirtualHostNodeImpl>
        implements RocksDBVirtualHostNode<RocksDBVirtualHostNodeImpl>
{
    /**
     * The managed object type name.
     */
    public static final String VIRTUAL_HOST_NODE_TYPE = "ROCKSDB";
    ...
    /**
     * Creates a RocksDB virtual host node.
     *
     * @param attributes configuration attributes.
     * @param parent the broker.
     */
    ...
    public RocksDBVirtualHostNodeImpl(final Map<String, Object> attributes, final Broker<?> parent)
    {
        super(attributes, parent);
    }

    /**
     * Writes a location event for the configuration store.
     */
    @Override
    ...
    /**
     * Creates the configuration store for the virtual host node.
     *
    ...
    {
        RocksDBConfigurationStore store = getStore();
        return store == null ? null : store.getPreferenceStore();
    }

    /**
     * Returns the supported child types.
     *
     * @return the supported child types.
     */
    ...
    {
        if (getConfigurationStore() instanceof RocksDBConfigurationStore)
        {
            return ((RocksDBConfigurationStore) getConfigurationStore()).getEnvironment();
        }
        return null;
    }
    ...
    /**
     * Returns the RocksDB configuration store when available.
     *
     * @return the configuration store or null when unavailable.
     */
    private RocksDBConfigurationStore getStore()
    {
        DurableConfigurationStore store = getConfigurationStore();
        if (store instanceof RocksDBConfigurationStore)
        {
            return (RocksDBConfigurationStore) store;
        }
        return null;
    }
