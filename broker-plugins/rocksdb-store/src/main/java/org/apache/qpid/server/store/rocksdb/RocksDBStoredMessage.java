        ...
        implements StoredMessage<T>, MessageHandle<T>
{
                         ...
                         final boolean stored)
    {
        ...
        T metaData = (T) record.getMetaData();
        _metaData = metaData;
        _metadataSize = metaData.getStorableSize();
        _contentSize = metaData.getContentSize();
        _chunked = record.isChunked();
        _chunkSize = record.getChunkSize();
    ...
    public synchronized T getMetaData()
    {
        if (_metaData == null)
        {
            ...
            @SuppressWarnings("unchecked")
            T metaData = record == null ? null : (T) record.getMetaData();
            _metaData = metaData;
            if (record != null)
            {
                _chunked = record.isChunked();
                if (_chunked && _chunkSize <= 0)
                {
                    _chunkSize = record.getChunkSize() > 0 ? record.getChunkSize() : _access.getDefaultChunkSize();
                }
            }
            _access.getInMemorySize().addAndGet(_metadataSize);
        }
