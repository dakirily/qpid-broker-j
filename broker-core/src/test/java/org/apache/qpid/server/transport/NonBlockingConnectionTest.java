...
class NonBlockingConnectionTest
{
    ...
    void beforeEach()
    {
        ...
        when(port.getContextValue(Integer.class, AmqpPort.FINAL_WRITE_THRESHOLD)).thenReturn(10);
        when(port.getContextValue(Long.class, AmqpPort.FINAL_WRITE_TIMEOUT)).thenReturn(100L);
        final SelectorThread.SelectionTask selectionTask = mock(SelectorThread.SelectionTask.class);
    ...
    void shutdownFinalWriteLooping() throws Exception
    {
        ...
        assertEquals("Failed to perform final write to connection after 100 ms timeout", secondLogEntry.getArgumentArray()[1]);
    }
